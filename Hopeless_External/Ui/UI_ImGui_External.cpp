...
namespace UIImGui
{
    ...
    void UIImGui_External::NewWindow(std::string WindowName, Vec2 WindowSize, std::function<void()> CallBack)
    {
        if (!CallBack)
            throw UIException("CallBack is empty");
        if (WindowName.empty())
            Window.Name = "Window";
        ...
        if (!CreateMyWindow())
            throw UIException("CreateMyWindow() call failed");
    ...
    void  UIImGui_External::AttachAnotherWindow(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        if (!CallBack)
            throw UIException("CallBack is empty");
        if (DestWindowName.empty() && DestWindowClassName.empty())
            throw UIException("DestWindowName and DestWindowClassName are empty");
        ...
        if (DestWindow.hWnd == NULL)
            throw UIException("DestWindow isn't exist");
    ...
    bool UIImGui_External::PeekEndMessage()
    {
        MSG msg;
    ...
    void UIImGui_External::MainLoop()
    {
        while (!EndFlag)
        {
            if (PeekEndMessage())
    ...
    bool UIImGui_External::CreateMyWindow()
    {
    ...
    bool UIImGui_External::UpdateWindowData()
    {
        POINT Point{};
        RECT Rect{};
